...
| valkey.auth.aclUsers.default | object | `{"passwordKey":"redis-password","permissions":"on ~* allchannels +@read +@write +ping +info +psync +replconf +@hash +@list +@pubsub +@scripting +TIME"}` | Enable default Valkey user. [Documentation](https://github.com/valkey-io/valkey-helm) |
| valkey.auth.aclUsers.default.passwordKey | string | `"redis-password"` | Set the name for the Secret field where the password for the default Valkey user is stored. [Helm Existing Secret](https://github.com/valkey-io/valkey-helm/tree/main/valkey#existing-secret-recommended) |
...
| valkey.auth.usersExistingSecret | string | `"my-redis-secret"` | Set name of existing Kubernetes secret |
